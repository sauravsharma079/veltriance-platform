    ...
    <div className="min-h-screen bg-[#F7F8FA]">
      ...
      <div className="px-8 py-6">
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[["Active",active,CheckCircle,"text-emerald-600","bg-emerald-50"],["Preferred",pref,Star,"text-amber-600","bg-amber-50"],["High Risk",hi,AlertTriangle,"text-red-600","bg-red-50"],["Avg Rating",avg||"—",BarChart3,"text-[#1A2A52]","bg-[#1A2A52]/8"]].map(([l,v,I,tc,bg])=>(
            <div key={String(l)} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex items-center gap-3"><div className={`size-10 ${bg} rounded-xl flex items-center justify-center`}><I className={`size-5 ${tc}`}/></div><div><p className="text-xl font-bold text-gray-900">{String(v)}</p><p className="text-[10px] text-gray-400">{String(l)}</p></div></div>
          ))}
        </div>
        <div className="flex gap-3 mb-5">
          ...
          <select value={st} onChange={e=>setSt(e.target.value)} className="text-xs border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-[#1A2A52]"><option value="">All</option><option value="ACTIVE">Active</option><option value="PENDING_APPROVAL">Pending</option><option value="INACTIVE">Inactive</option></select>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          <div className="grid grid-cols-12 gap-3 px-5 py-3 bg-gray-50 border-b text-[10px] font-semibold text-gray-400 uppercase tracking-wide"><div className="col-span-3">Supplier</div><div className="col-span-2">Category</div><div className="col-span-1">Tier</div><div className="col-span-1">Status</div><div className="col-span-2">Rating</div><div className="col-span-2">Risk</div><div className="col-span-1">Contact</div></div>
          ...
          :list.map(s=>(
            <Link key={s.id} href={`/dashboard/suppliers/${s.id}`} className="grid grid-cols-12 gap-3 px-5 py-3.5 border-b border-gray-50 last:border-0 items-center hover:bg-gray-50/60 transition-colors">
              ...
              <div className="col-span-1"><p className="text-[10px] text-gray-400 truncate">{s.contactName||"—"}</p></div>
            </Link>
          ))}
        </div>
