...
export async function GET(_: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const S = po.supplier as any;
    const O = (po as any).organization as any;
    const LI = (po as any).lineItems as any[];
...
<title>Purchase Order — ${(po as any).poNumber}</title>
<style>
...
<div class="page">
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:16px;border-bottom:3px solid #1A2A52;margin-bottom:20px">
    <div>
      <div style="font-size:24px;font-weight:900;color:#1A2A52;letter-spacing:-0.5px">Veltriance</div>
      <div style="font-size:8px;color:#C8A04D;letter-spacing:2.5px;text-transform:uppercase;margin-top:2px">Technologies LLP</div>
      <div style="font-size:9px;color:#6b7280;margin-top:10px;line-height:1.7">
        Procurement Platform<br>
        ${O?.name || "Ace Technologies India Pvt. Ltd."}<br>
        CIN: U72900KA2024PTC000001
      </div>
    </div>
    <div style="text-align:right">
      <div style="font-size:28px;font-weight:900;color:#1A2A52;letter-spacing:1px">PURCHASE ORDER</div>
      <div style="font-size:14px;font-weight:700;color:#C8A04D;margin-top:4px">${(po as any).poNumber}</div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px">
    ...
    <div style="background:#f8f9fb;border-radius:6px;padding:12px;border-left:3px solid #1A2A52">
      ...
      <div style="font-size:9px;color:#6b7280;margin-top:2px;line-height:1.6">Required by: ${dt((po as any).requiredDate)}<br>Payment: ${(po as any).paymentTerms || S?.paymentTerms || "Net 30"}<br>Currency: ${(po as any).currency || "INR"}</div>
    </div>
  </div>
  ...
  <div style="background:#f8f9fb;border-radius:6px;padding:12px;border-left:3px solid #C8A04D;margin-bottom:20px">
    ...
    <div style="font-size:11px;font-weight:600;color:#111">${(po as any).deliveryLocation || "To be confirmed"}</div>
  </div>
  ...
  <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
    ...
    <tbody>
      ${rows || '<tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:20px;font-size:10px">No line items</td></tr>'}
    </tbody>
  </table>
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:20px">
    <div style="width:260px">
      <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #e5e7eb;font-size:10px;color:#374151">
        <span>Subtotal (excl. GST)</span><span style="font-family:monospace">${fmt((po as any).subtotal ?? 0)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #e5e7eb;font-size:10px;color:#374151">
        <span>IGST @ 18%</span><span style="font-family:monospace">${fmt((po as any).totalTax ?? 0)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:8px 0 6px;font-weight:700;font-size:13px;color:#1A2A52;border-top:2px solid #1A2A52;margin-top:4px">
        <span>TOTAL AMOUNT</span><span style="font-family:monospace">${fmt((po as any).totalAmount ?? 0)}</span>
      </div>
    </div>
  </div>
  ...
  <div style="background:#f0f4ff;border-radius:6px;padding:14px;margin-bottom:24px;border:1px solid #dbeafe">
    ...
    <p style="font-size:9px;color:#374151;line-height:1.8">
      ...
      3. Invoice must quote PO number <strong>${(po as any).poNumber}</strong>. Invoices without reference will not be processed.<br>
      4. Payment will be made as per agreed terms upon receipt of goods/services and valid tax invoice.<br>
      5. Supplier warrants that goods/services comply with all applicable laws and are free from defects.
    </p>
  </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:24px;border-top:2px solid #e5e7eb;padding-top:24px">
    <div style="text-align:center">
      <div style="height:44px;border-bottom:1.5px solid #374151;margin-bottom:8px"></div>
      <div style="font-size:10px;font-weight:600;color:#111">Procurement Manager</div>
      <div style="font-size:9px;color:#6b7280;margin-top:2px">Authorised Signatory</div>
    </div>
    <div style="text-align:center">
      <div style="height:44px;border-bottom:1.5px solid #374151;margin-bottom:8px"></div>
      <div style="font-size:10px;font-weight:600;color:#111">Finance Controller</div>
      <div style="font-size:9px;color:#6b7280;margin-top:2px">Finance Approval</div>
    </div>
    <div style="text-align:center">
      <div style="height:44px;border-bottom:1.5px solid #374151;margin-bottom:8px"></div>
      <div style="font-size:10px;font-weight:600;color:#111">${S?.contactName || "Supplier Representative"}</div>
      <div style="font-size:9px;color:#6b7280;margin-top:2px">Supplier Acknowledgement</div>
    </div>
  </div>

  <!-- FOOTER -->
  <div style="text-align:center;font-size:8.5px;color:#9ca3af;margin-top:20px;padding-top:12px;border-top:1px solid #f3f4f6">
    Generated by Veltriance Procurement Platform &nbsp;|&nbsp; ${(po as any).poNumber} &nbsp;|&nbsp; ${new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}<br>
  ...
  window.addEventListener("load", function() {
    ...
    return new NextResponse(html, {
      headers: {
        ...
        "Content-Disposition": `inline; filename="PO-${(po as any).poNumber}.html"`,
    ...
    return new NextResponse(`<html><body style="font-family:Arial;padding:40px;color:red">Error: ${e?.message}</body></html>`, {
      status: 500,
